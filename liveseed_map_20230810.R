...
#Map or Organi Plant Breeding Initatives
#Call the libraries
library(leaflet)
library(sp)
library(rgdal)
...
addMarkers(
  data = data, lng = ~long, lat = ~lat, label = data$name,
  group = 'data', # this is the group to use in addSearchFeatures()
  ...
  addSearchFeatures(
    targetGroups = 'data', # group should match addMarkers() group
